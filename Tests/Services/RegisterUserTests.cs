...
namespace V1.Tests.Services
{
    ...
    public class RegisterUserTests : UserServiceTests
    {
        ...
        /// </summary>
        private readonly CreateUserDTO request;
        ...
        public async Task CreateUser_ShouldThrowException_WhenUsernameAlreadyExists()
        {
            ...
            _userRepositoryMock
                .Setup(repo => repo.UsernameExists(request.Username))
                .ReturnsAsync(true);
            ...
            _userRepositoryMock.Verify(repo => repo.UsernameExists(request.Username), Times.Once);

            // Verify that HashPassword was never called, as the process should halt after the username check fails
            _passwordHasherMock.Verify(hasher => hasher.HashPassword(request.Password), Times.Never);
